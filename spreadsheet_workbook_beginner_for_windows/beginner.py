...
def display_rank_color(accuracy):
    if accuracy == 1:
        rank = "プレデター"
        rank_color = "maroon"
    elif 0.8 <= accuracy < 1:
    ...
    colored_rank = f'<p style="color:{rank_color};">{rank}</p>'
    components.v1.html(colored_rank, height=50)
...
def display_question_screen():
    ...
    if st.button("解答", key="answer_button"):
    ...
    # 解説ボタンが押されたら、解説を表示する
    if st.button("解説", key="explanation_button"):
...
def display_result_screen():
    ...
    if st.session_state.question_index < 20:
        st.write("問題の解答数が足りてないのでランクをつけれません")
    else:
        # ランク表示
    ...
    for result in st.session_state.results:
        col1.markdown(f"<span style='font-size: 0.7em;'>{result['question']}</span>", unsafe_allow_html=True)
        if result["is_correct"]:
            col2.markdown("<span style='color: blue;'>&#x25CB;</span>", unsafe_allow_html=True)  # 正解は青色の○
        else:
            col2.markdown("<span style='color: red;'>&#x2715;</span>", unsafe_allow_html=True)  # 不正解は赤色の×
